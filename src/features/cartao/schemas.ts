...
export const createCompraCartaoSchema = z.object({
  contaId: z.string().min(1, 'contaId obrigatório').max(100),
  descricao: z.string().trim().min(1, 'Descrição obrigatória').max(255, 'Descrição muito longa'),
  categoriaId: z.string().min(1, 'categoriaId inválido').max(100).optional().nullable(),
  categoria: z.string().trim().min(1, 'Categoria obrigatória').max(100),
  valorTotalCentavos: z.number({ required_error: 'Valor obrigatório' })
    .int('Valor deve ser inteiro em centavos')
    .positive('Valor deve ser maior que zero'),
  dataCompra: z.string().regex(DATE_REGEX, 'Data inválida - formato: YYYY-MM-DD'),
...
export const faturaCartaoFiltersSchema = z.object({
  ...
  competencia: z.string().regex(COMPETENCIA_REGEX, 'Competência inválida - formato: YYYY-MM').optional(),
}).strict()

export const pagarFaturaSchema = z.object({
  faturaCartaoId: z.string().min(1, 'faturaCartaoId obrigatório').max(100),
  contaPagamentoId: z.string().min(1, 'contaPagamentoId obrigatório').max(100),
  valorCentavos: z.number({ required_error: 'Valor obrigatório' })
    .int('Valor deve ser inteiro em centavos')
    .positive('Valor deve ser maior que zero'),
  dataPagamento: z.string().regex(DATE_REGEX, 'Data inválida - formato: YYYY-MM-DD'),
}).strict()
