'use client'

import { Loader2, Pencil, Trash2, CheckCircle, AlertTriangle } from 'lucide-react'
import { cn } from '@/lib/utils'
import { fmtBRL, RAS_STATUS_LABELS, RAS_STATUS_COLORS } from '@/types/ras'
import type { RasAgenda, StatusRas } from '@/types/ras'

// ─── Helpers ─────────────────────────────────────────────────────────────────

function dateToBR(iso: string): string {
  const [y, m, d] = iso.slice(0, 10).split('-')
  return `${d}/${m}/${y}`
}

function formatDateTime(iso: string): string {
  if (!iso) return '—'
  const date = new Date(iso)
  const d   = String(date.getDate()).padStart(2, '0')
  const m   = String(date.getMonth() + 1).padStart(2, '0')
  const y   = date.getFullYear()
  const h   = String(date.getHours()).padStart(2, '0')
  const min = String(date.getMinutes()).padStart(2, '0')
  return `${d}/${m}/${y} ${h}:${min}`
}

// Retorna a data de cancelamento: deletadoEm se disponível, senão updatedAt (quando o status mudou)
function canceladoEm(ras: RasAgenda): string {
  return ras.deletadoEm ?? ras.updatedAt
}

function localShort(local: string): string {
  return local.includes(' - ') ? local.split(' - ')[0] : local
}

// ─── Badges ──────────────────────────────────────────────────────────────────

function StatusBadge({ status }: { status: StatusRas }) {
  const colors = RAS_STATUS_COLORS[status]
  return (
    <span
      className="inline-flex items-center rounded-full text-[10px] px-2 py-0.5 font-semibold whitespace-nowrap"
      style={{ color: colors.color, background: colors.bg }}
    >
      {RAS_STATUS_LABELS[status]}
    </span>
  )
}

function TipoBadge({ tipo, tipoVaga }: { tipo: string; tipoVaga: string }) {
  const isVol = tipo === 'voluntario'
  return (
    <span
      className="inline-flex items-center rounded text-[10px] px-1.5 py-0.5 font-semibold whitespace-nowrap"
      style={
        isVol
          ? { background: 'rgba(139,92,246,.12)', color: '#8b5cf6' }
          : { background: 'rgba(245,158,11,.12)', color: '#d97706' }
      }
    >
      {isVol ? 'Vol' : 'Comp'} · {tipoVaga === 'titular' ? 'Titular' : 'Reserva'}
    </span>
  )
}

// ─── Props ────────────────────────────────────────────────────────────────────

interface RasListProps {
  rasAgendas: RasAgenda[]
  isLoading: boolean
  onEdit: (ras: RasAgenda) => void
  onDelete: (id: string) => void
  onMarcarRealizado: (id: string) => void
  isReadOnly?: boolean
}

// ─── Component ────────────────────────────────────────────────────────────────

export function RasList({
  rasAgendas,
  isLoading,
  onEdit,
  onDelete,
  onMarcarRealizado,
  isReadOnly = false,
}: RasListProps) {
  if (isLoading) {
    return (
      <div className="rounded-xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#1C1C1C] shadow-sm">
        <div className="px-5 py-10 flex flex-col items-center gap-3 text-gray-400">
          <Loader2 size={24} className="animate-spin" aria-hidden />
          <p className="text-sm">Carregando RAS…</p>
        </div>
      </div>
    )
  }

  if (rasAgendas.length === 0) return null

  // isReadOnly = aba cancelados: mostra todos os itens passados (já filtrados no page)
  // caso contrário, exclui cancelados por segurança
  const active = (isReadOnly ? rasAgendas : rasAgendas.filter((r) => r.status !== 'cancelado'))
    .sort((a, b) => a.data.localeCompare(b.data))

  return (
    <div className="rounded-xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#1C1C1C] shadow-sm overflow-hidden">
      {/* Header */}
      <div className="px-5 py-3.5 border-b border-gray-100 dark:border-white/[0.06] flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Registros</h2>
        <span className="text-xs text-gray-400 dark:text-gray-500 tabular-nums">
          {active.length} {active.length === 1 ? 'registro' : 'registros'}
        </span>
      </div>

      {/* ── Desktop table ─────────────────────────────────────────────────── */}
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm" style={{ minWidth: 680 }}>
          <thead>
            <tr className="bg-gray-50 dark:bg-white/[0.03]">
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-5 py-3">Data</th>
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Hora</th>
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Dur.</th>
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Tipo</th>
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Grad.</th>
              <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Local</th>
              <th className="text-right text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3 whitespace-nowrap">Valor</th>
              {!isReadOnly && (
                <th className="text-center text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Status</th>
              )}
              {isReadOnly && (
                <th className="text-left text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3 whitespace-nowrap">Cancelado em</th>
              )}
              {!isReadOnly && (
                <th className="text-center text-xs font-medium text-gray-400 dark:text-gray-500 px-4 py-3">Ações</th>
              )}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50 dark:divide-white/[0.04]">
            {active.map((ras) => {
              const canEdit     = ras.status === 'agendado'
              const canDelete   = ras.status !== 'confirmado' && ras.status !== 'cancelado'
              const canRealizar = ras.status === 'agendado'

              return (
                <tr
                  key={ras.id}
                  className="hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors"
                >
                  <td className="px-5 py-3.5 text-xs font-semibold text-gray-800 dark:text-gray-200 whitespace-nowrap">
                    {dateToBR(ras.data)}
                  </td>
                  <td className="px-4 py-3.5 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                    {ras.horaInicio}
                  </td>
                  <td className="px-4 py-3.5 text-xs font-semibold text-gray-600 dark:text-gray-300">
                    {ras.duracao}h
                  </td>
                  <td className="px-4 py-3.5">
                    <TipoBadge tipo={ras.tipo} tipoVaga={ras.tipoVaga} />
                  </td>
                  <td className="px-4 py-3.5 text-xs text-gray-500 dark:text-gray-400">
                    {ras.graduacao}
                  </td>
                  <td className="px-4 py-3.5 text-xs text-gray-600 dark:text-gray-300 max-w-[150px] truncate">
                    {localShort(ras.local)}
                  </td>
                  <td className="px-4 py-3.5 text-right text-xs font-semibold text-emerald-600 dark:text-emerald-400 whitespace-nowrap tabular-nums">
                    {fmtBRL(ras.valorCentavos)}
                  </td>
                  {!isReadOnly && (
                    <td className="px-4 py-3.5 text-center">
                      <StatusBadge status={ras.status as StatusRas} />
                    </td>
                  )}
                  {isReadOnly && (
                    <td className="px-4 py-3.5 text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">
                      {formatDateTime(canceladoEm(ras))}
                    </td>
                  )}
                  {!isReadOnly && (
                    <td className="px-4 py-3.5">
                      <div className="flex items-center justify-center gap-1">
                        {canRealizar && (
                          <button
                            onClick={() => onMarcarRealizado(ras.id)}
                            title="Marcar como realizado"
                            className="w-7 h-7 rounded-md flex items-center justify-center text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors"
                          >
                            <CheckCircle size={14} aria-hidden />
                          </button>
                        )}
                        {canEdit && (
                          <button
                            onClick={() => onEdit(ras)}
                            title="Editar"
                            className="w-7 h-7 rounded-md flex items-center justify-center text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                          >
                            <Pencil size={14} aria-hidden />
                          </button>
                        )}
                        {canDelete && (
                          <button
                            onClick={() => onDelete(ras.id)}
                            title="Cancelar RAS"
                            className="w-7 h-7 rounded-md flex items-center justify-center text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                          >
                            <Trash2 size={14} aria-hidden />
                          </button>
                        )}
                      </div>
                    </td>
                  )}
                </tr>
              )
            })}
          </tbody>
        </table>
      </div>

      {/* ── Mobile rich cards ─────────────────────────────────────────────── */}
      <ul className="md:hidden divide-y divide-gray-100 dark:divide-white/[0.05]">
        {active.map((ras) => {
          const canRealizar = ras.status === 'agendado'
          const canEdit     = ras.status === 'agendado'
          const canDelete   = ras.status !== 'confirmado' && ras.status !== 'cancelado'
          const isPendente  = ras.status === 'pendente'

          return (
            <li
              key={ras.id}
              className={cn(
                'px-4 py-4',
                isPendente && 'bg-amber-50/40 dark:bg-amber-900/10'
              )}
            >
              {/* Linha 1: data + hora/duração + valor */}
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-bold text-gray-900 dark:text-gray-100">
                    {dateToBR(ras.data)}
                  </span>
                  <span className="text-xs text-gray-400 dark:text-gray-500">
                    {ras.horaInicio} · {ras.duracao}h
                  </span>
                  {isPendente && (
                    <AlertTriangle size={12} className="text-amber-500" aria-hidden />
                  )}
                </div>
                <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400 tabular-nums shrink-0">
                  {fmtBRL(ras.valorCentavos)}
                </span>
              </div>

              {/* Linha 2: local + graduação + tipo */}
              <div className="flex items-center gap-1.5 flex-wrap mb-3">
                <span className="text-xs text-gray-500 dark:text-gray-400">
                  {localShort(ras.local)}
                </span>
                <span className="text-gray-300 dark:text-gray-600 text-xs">·</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">{ras.graduacao}</span>
                <TipoBadge tipo={ras.tipo} tipoVaga={ras.tipoVaga} />
              </div>

              {/* Linha 3: status + pill action buttons */}
              <div className="flex items-center gap-2 flex-wrap">
                {!isReadOnly && <StatusBadge status={ras.status as StatusRas} />}
                {isReadOnly && (
                  <span className="text-xs text-gray-400 dark:text-gray-500">
                    Cancelado em: {formatDateTime(canceladoEm(ras))}
                  </span>
                )}

                {!isReadOnly && (
                  <div className="flex items-center gap-1.5 ml-auto">
                    {canRealizar && (
                      <button
                        onClick={() => onMarcarRealizado(ras.id)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-900/50 transition-colors"
                      >
                        <CheckCircle size={11} aria-hidden />
                        Realizado
                      </button>
                    )}
                    {canEdit && (
                      <button
                        onClick={() => onEdit(ras)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
                      >
                        <Pencil size={11} aria-hidden />
                        Editar
                      </button>
                    )}
                    {canDelete && (
                      <button
                        onClick={() => onDelete(ras.id)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                      >
                        <Trash2 size={11} aria-hidden />
                        Cancelar
                      </button>
                    )}
                  </div>
                )}
              </div>
            </li>
          )
        })}
      </ul>
    </div>
  )
}
